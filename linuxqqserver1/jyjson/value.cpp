...
bool ValueObj::clean()//清理整个ValueObj对象
{
	...
	for (; b != e; ++b)
		b->second.clean();
		//clean(b->second.type,b->second.vpoint);
...
ValueObj& ValueObj::operator =(const ValueObj &org)
{
	if (&org != this)
	{
		std::map<std::string, TypeValue>::const_iterator b = org.mypair.begin();
...
ValueArr& ValueArr::operator =(const ValueArr &org)
{
	if (&org != this)
	{
		std::vector<TypeValue>::const_iterator b = org.vec.begin();
...
//const char* TypeValue::getchar()const
//{	
//	if (type!=jyjson_string)
//	{
//		throw jsexception("type is not match");
//	}
//	return (  *((std::string*)vpoint) ).c_str();
//}
...
//TypeValue::operator const char*()const
//{
	//return getchar();
//}
...
/*TypeValue& TypeValue::operator=(ValueObj* a)
{
   clean(this->type,this->vpoint);
...
TypeValue& TypeValue::operator=(ValueArr* a)
{
	clean(this->type,this->vpoint);
...
/*TypeValue& TypeValue::operator=(void* a)//only NULL
{
	clean(this->type,this->vpoint);
   this->type=jyjson_null;
   this->vpoint=NULL;
   return *this;
}*/
